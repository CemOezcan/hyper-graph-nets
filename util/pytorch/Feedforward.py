...
def get_feedforward_layout(max_neurons: int, num_layers: int, network_shape: str = "block") -> np.ndarray:
    """
...
class Feedforward(nn.Module):
    def __init__(self, feedforward_config: ConfigDict, in_features: int, out_features: Optional[int] = None):
        super().__init__()
        ...
        for current_layer_size in network_layout:
            self.feedforward_layers.append(nn.Linear(in_features=previous_shape,
                                                     out_features=current_layer_size))
            previous_shape = current_layer_size
            self.feedforward_layers.append(nn.LeakyReLU())

        if out_features is not None:
            self.feedforward_layers.append(nn.Linear(previous_shape, out_features))
